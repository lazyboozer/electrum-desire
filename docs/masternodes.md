...
Electrum-DSR supports masternode creation through an interface called the Masternode Manager.
...
- The DSR address that received the collateral payment.
- The DSR address of the delegate key.
...
A masternode requires a "delegate" key, which is known to both Electrum-DSR and your masternode.
Your masternode will use this key to sign messages, and the DSR network will know that you authorized
it to.

A delegate key can either be one of your Electrum-DSR keys, or an imported key. Either way, your masternode
and Electrum-DSR will both need to know the private key. (See *Importing Masternode.conf* below.)

To use one of your Electrum-DSR keys as a delegate key, put its address in the `Masternode DSR Address`
...
### Collateral (1000 DSR Payment)

To start a masternode, you must have a 1000 DSR payment available in your wallet.
You can scan your wallet for 1000 DSR payments in the `Activate Masternode` tab of the Masternode
Manager.

After scanning, a list of available 1000 DSR collateral payments will be displayed. Selecting one
...
waiting for Electrum-DSR to sign and broadcast your masternode announcement, you will be presented with
