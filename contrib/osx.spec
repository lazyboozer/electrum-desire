...
datas = [
    ...
    ('lib/currencies.json', 'electrum_desire'),
    ('lib/wordlist', 'electrum_desire/wordlist'),
]
...
# Add TOC to electrum_desire, electrum_desire_gui, electrum_desire_plugins
for p in sorted(a.pure):
    if p[0].startswith('lib') and p[2] == 'PYMODULE':
        a.pure += [('electrum_desire%s' % p[0][3:] , p[1], p[2])]
    if p[0].startswith('gui') and p[2] == 'PYMODULE':
        a.pure += [('electrum_desire_gui%s' % p[0][3:] , p[1], p[2])]
    if p[0].startswith('plugins') and p[2] == 'PYMODULE':
        a.pure += [('electrum_desire_plugins%s' % p[0][7:] , p[1], p[2])]
...
exe = EXE(pyz,
          ...
          upx=False,
          console=False,
          icon='icons/electrum-desire.ico',
          name=os.path.join('build/electrum-desire/electrum-desire', cmdline_name))
...
tctl_exe = EXE(tctl_pyz,
           ...
           upx=False,
           console=True,
           name=os.path.join('build/electrum-desire/electrum-desire', 'trezorctl.bin'))
...
coll = COLLECT(exe, tctl_exe,
               ...
               strip=False,
               upx=False,
               name=os.path.join('dist', 'electrum-desire'))

app = BUNDLE(coll,
             name=os.path.join('dist', 'Electrum-DSR.app'),
             appname="Electrum-DSR",
	         icon='electrum-desire.icns',
