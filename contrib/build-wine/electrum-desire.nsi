...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-DSR"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-desire-${PRODUCT_VERSION}-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "icons\electrum-desire.ico"
...
Section
  ...
  File /r "dist\electrum-desire\*.*"
  File "icons\electrum-desire.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-desire-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-desire-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-desire-${PRODUCT_VERSION}.exe" 0

  ;Links desire: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\desire" "" "URL:desire Protocol"
  WriteRegStr HKCU "Software\Classes\desire" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\desire" "DefaultIcon" "$\"$INSTDIR\electrum-desire.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\desire\shell\open\command" "" "$\"$INSTDIR\electrum-desire-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\electrum-desire.ico"
