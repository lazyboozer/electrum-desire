...
from electrum_desire import Wallet, WalletStorage
from electrum_desire.util import UserCancelled, InvalidPassword
from electrum_desire.base_wizard import BaseWizard
...
MSG_GENERATING_WAIT = _("Electrum-DSR is generating your addresses, please wait...")
MSG_ENTER_ANYTHING = _("Please enter a seed phrase, a master key, a list of "
                       "Desire addresses, or a list of private keys")
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def __init__(self, config, app, plugins, storage):
        ...
        self.setWindowTitle('Electrum-DSR  -  ' + _('Install Wizard'))
        self.app = app
        self.config = config
        ...
        self.set_icon(':icons/electrum-desire.png')
        self.show()
        self.raise_()
    ...
    def run_and_get_wallet(self):
        ...
        self.set_layout(vbox, title=_('Electrum-DSR wallet'))
        ...
        if self.storage.requires_split():
            self.hide()
            msg = _("The wallet '%s' contains multiple accounts, which are no longer supported in Electrum-DSR 2.7.\n\n"
        ...
        if self.storage.requires_upgrade():
            self.hide()
            msg = _("The format of your wallet '%s' must be upgraded for Electrum-DSR. This change will not be backward compatible"%path)
            if not self.question(msg):
                return
    ...
    def init_network(self, network):
        message = _("Electrum-DSR communicates with remote servers to get "
                  "information about your transactions and addresses. The "
                  "servers all fulfil the same purpose only differing in "
                  "hardware. In most cases you simply want to let Electrum-DSR "
